...
class twostageHDR:
    def __init__(self, config):
        ...
        # Optimizers
        self.optimizerEG = torch.optim.Adam(
            self.attentionNet.parameters(),
            lr=self.learningRate,
            betas=(self.adamBeta1, self.adamBeta2),
        )
        self.optimizerER = torch.optim.Adam(
            self.HDRRec.parameters(),
            lr=self.learningRate,
            betas=(self.adamBeta1, self.adamBeta2),
        )
        self.optimizerED = torch.optim.Adam(
    ...
    def modelTraining(self, resumeTraning=False, overFitTest=False, dataSamples=None):
        ...
        if overFitTest == True:
            customPrint(
                Fore.RED + "Over Fitting Testing with an arbitary image!", self.barLen
            )
        ...
        if resumeTraning == True:
            ...
            try:
                ...
                customPrint(
                    Fore.RED
                    + "Would you like to start training from sketch (default: Y): ",
        ...
        # Starting Training
        customPrint(
            "Training is about to begin using:"
            + Fore.YELLOW
            + "[{}]".format(self.device).upper(),
            textWidth=self.barLen,
        )
        ...
        self.scheduleLG = optim.lr_scheduler.StepLR(
            self.optimizerEG, step_size=1, gamma=0.01
        )  # optim.lr_scheduler.OneCycleLR(optimizer=self.optimizerEG, max_lr=self.learningRate, total_steps=self.totalSteps)
        self.scheduleLR = optim.lr_scheduler.StepLR(
            self.optimizerER, step_size=1, gamma=0.01
        )
        ...
        while currentStep < self.totalSteps:
            ...
            for LRImagesLeft, HRGTImages, HRGTImages16 in trainingImageLoader:
                ...
                if currentStep > self.totalSteps:
                    ...
                    customPrint(
                        Fore.YELLOW + "Training Completed Successfully!",
                        textWidth=self.barLen,
                    )
                    exit()
                ...
                targetReal = (torch.rand(rawInputLeft.shape[0], 1) * 0.5 + 0.7).to(
                    self.device
                )
                targetFake = (torch.rand(rawInputLeft.shape[0], 1) * 0.3).to(
                    self.device
                )
                ...
                generatorAdversarialLoss = adversarialLoss(
                    self.discriminator(highResFake), onesConst
                )
                ...
                lossED = adversarialLoss(
                    self.discriminator(highResReal), targetReal
                ) + adversarialLoss(
                    self.discriminator(highResFake.detach()), targetFake
                )
                lossED.backward()
                ...
                psnr = (
                    10
                    * torch.log(MAX_DIFF**2 / mseLoss(highResHDR, highResReal16))
                    / log10
                )
                generatorContentLoss = reconstructionLoss(
                    highResHDR, highResReal16
                ) + colorLoss(highResHDR, highResReal16)
                ...
                if (currentStep + 1) % 25 == 0:
                    ...
                    print(
                        ...
                        bar,
                        Fore.YELLOW
                        + "| LossEG: {:.4f}, LossER: {:.4f}, outMax: {:.4f}, gtMax: {:.4f}, PSNR: {:.4f}".format(
                            lossEG, lossER, highResHDR.max(), highResReal16.max(), psnr
                        ),
                        end="\r",
                    )
                ...
                if (currentStep + 1) % 10000 == 0:
                    ...
                    print(
                        Fore.CYAN
                        + "Steps [{}/{}] | Time elapsed [{:0>2}:{:0>2}:{:0>2}] | OutMax: {:.2f}, GTMax: {:.2f}, PSNR: {:.2f}, LossEG: {:.2f}, LossER: {:.2f}, LossED: {:.2f}".format(
                            currentStep + 1,
                            self.totalSteps,
                            eHours,
                            eMinutes,
    ...
    def modelInference(
        ...
        print(
            f"modelInference: testImagesPath={self.testImagesPath}, outputDir={self.resultDir}, modelName={self.modelName}, validation={validation}"
        )
        ...
        with torch.no_grad():
            for imgPath in testImageList:
                ...
                imgLDR, lumLDR = modelInference.inputForInference(
                    imgPath, noiseLevel=0
                )  # .to(self.device)
                ...
                modelInference.saveModelOutput(hdrnet_output, imgPath, steps)
    ...
    def modelSummary(self, input_size=None):
        if not input_size:
            input_size = (
                3,
                self.imageH // self.scalingFactor,
                self.imageW // self.scalingFactor,
            )
        ...
        flops, params = get_model_complexity_info(
            self.attentionNet, input_size, as_strings=True, print_per_layer_stat=False
        )
        customPrint(
            "Computational complexity (Enhace-Gen):{}".format(flops), self.barLen, "-"
        )
        customPrint(
            "Number of parameters (Enhace-Gen):{}".format(params), self.barLen, "-"
        )

        flops, params = get_model_complexity_info(
            self.discriminator, input_size, as_strings=True, print_per_layer_stat=False
        )
        customPrint(
            "Computational complexity (Enhace-Dis):{}".format(flops), self.barLen, "-"
        )
        customPrint(
            "Number of parameters (Enhace-Dis):{}".format(params), self.barLen, "-"
        )
        print()
    ...
    def savingWeights(self, currentStep, duplicate=None):
        ...
        saveCheckpoint(
            modelStates=checkpoint, path=self.checkpointPath, modelName=self.modelName
        )
        if duplicate:
            saveCheckpoint(
                modelStates=checkpoint,
                path=self.checkpointPath + str(currentStep) + "/",
                modelName=self.modelName,
                backup=None,
            )
